...
- Вход через Telegram Login Widget. По умолчанию фронтенд использует login-бота `@asd20342bot`, если backend не отдал своего бота из `/api/bootstrap`.
...
2. Заполните Telegram-настройки в `.env`:
   ...
   - `TELEGRAM_LOGIN_BOT_USERNAME` — username бота без обязательного `@`, используется виджетом входа. Для текущего контура используется `asd20342bot`.
...
Фронтенд получает имя login-бота из `GET /api/bootstrap`, рендерит Telegram Login Widget и отправляет полученный payload в `POST /api/auth/telegram`. Если backend пока не настроен, интерфейс использует `@asd20342bot` и позволяет вручную сменить username бота без пересборки.
